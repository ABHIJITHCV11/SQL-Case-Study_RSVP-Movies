...
SELECT SUM(CASE WHEN id IS NULL THEN 1 ELSE 0 END) AS id_null_counts, 
	SUM(CASE WHEN title IS NULL THEN 1 ELSE 0 END) AS title_null_counts, 
	SUM(CASE WHEN date_published IS NULL THEN 1 ELSE 0 END) AS date_published_null_counts, 
	SUM(CASE WHEN duration IS NULL THEN 1 ELSE 0 END) AS duration_null_counts,
	SUM(CASE WHEN country IS NULL THEN 1 ELSE 0 END) AS country_null_counts,
	SUM(CASE WHEN worlwide_gross_income IS NULL THEN 1 ELSE 0 END) AS worlwide_gross_income_null_counts,
	SUM(CASE WHEN languages IS NULL THEN 1 ELSE 0 END) AS languages_null_counts,
	SUM(CASE WHEN production_company IS NULL THEN 1 ELSE 0 END) AS production_company_null_counts
FROM movie;
...
-- Type your code below:
SELECT relase_year,count(id) AS number_of_movies 
FROM movie
...
SELECT median_rating, count(movie_id) AS movies_with_8_median_rating
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
...
SELECT total_votes, languages AS movie_languages
FROM movie AS m
INNER JOIN ratings AS r
ON m.id = r.movie_id
